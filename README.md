...
- для перемещения используйте ручку или удерживайте пустое место панели одну секунду, если ручка
  отключена;
- нажатие на плитку топлива открывает уровень, запас хода, расчёт литров и время последнего ответа;
- кнопка сброса положения возвращает панель в безопасную область экрана.

## Данные топлива

Плитка и график топлива читают датчик Fuel Percentage из GInputBridge. Стандартная формула —
`литры = значение API × 0.5 + 4`; API передаёт значение от 0 до 100, поэтому максимальный объём
бака вычисляется по формуле и равен 54 л. В настройках можно задать собственные множитель и
смещение — объём бака также пересчитывается по этой формуле. При API=0 интерфейс показывает
оценочные границы: `<4` залитого и `>50` свободного топлива для стандартной формулы. Если
GInputBridge не отвечает или данные устарели, интерфейс показывает недоступное значение, а не
сохраняет его бессрочно.
